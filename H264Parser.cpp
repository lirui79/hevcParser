...
h264Parser::h264Parser() {
    m_h264_buffer = (streamBuffer*) malloc(sizeof(streamBuffer));
    memset(m_h264_buffer, 0, sizeof(streamBuffer));
}

h264Parser::~h264Parser() {
    if (m_h264_buffer != NULL) 
        free(m_h264_buffer);
    m_h264_buffer = NULL;
}
...
int h264Parser::h264_parser_Slice_Header(unsigned char * buffer, unsigned int bufferlen){
   ...
   int forbidden_zero_bit=bs_read_u(1,buffer, &StartBit);
   int nal_ref_idc=bs_read_u(2,buffer, &StartBit);
   int nal_unit_type=bs_read_u(5,buffer, &StartBit);
   int field_pic_flag;
   int picture_structure;
   int ret;
   int list_count;

   int first_mb_in_slice = bs_read_ue(buffer,bufferlen, &StartBit);
   int slice_type = bs_read_ue(buffer,bufferlen, &StartBit);//1:I Slice,0
   int pic_parameter_set_id = bs_read_ue(buffer,bufferlen, &StartBit);

   frame_num = bs_read_u(m_h264_paramter.sps[0].log2_max_frame_num,buffer, &StartBit);

   if(m_h264_paramter.sps[0].frame_mbs_only_flag){
	   picture_structure = 3;/*ffmpeg中PICT_FRAME*/
   }
   else{
	   field_pic_flag = bs_read_u(1,buffer, &StartBit);
	   if(field_pic_flag){
		   int bottom_field_flag = bs_read_u(1,buffer, &StartBit);
   ...
   if(nal_unit_type == 5){
	   idr_pic_id = bs_read_ue(buffer,bufferlen, &StartBit);/*idr_pic_id*/
   }

   if(m_h264_paramter.sps[m_h264_paramter.spslen-1].pic_order_cnt_type == 0){//第一种POC的求取方法
	   //log2_max_pic_order_cnt_lsb_minus4+4
	   pic_order_cnt_lsb = bs_read_u((m_h264_paramter.sps[m_h264_paramter.spslen-1].log2_max_pic_order_cnt_lsb_minus),buffer, &StartBit);
	   h264_getPOC();
	   if(m_h264_paramter.pps[m_h264_paramter.ppslen-1].pic_order_present_flag == 1 && picture_structure == 3){
		   /*delta_poc_bottom = */bs_read_se(buffer,bufferlen, &StartBit);
	   }
   	 }
	   ...
	   delta_pci_order_cnt[0] = bs_read_se(buffer,bufferlen, &StartBit);
	   if((m_h264_paramter.pps[m_h264_paramter.ppslen-1].pic_order_present_flag)&&(picture_structure == 3)){
		   delta_pci_order_cnt[1] = bs_read_se(buffer,bufferlen, &StartBit);
	   }
	   //
		//int h264_getPOC2();
   ...
   if((m_h264_paramter.pps[m_h264_paramter.ppslen-1].redundant_pic_cnt_present_flag)){
       ...
       bs_read_ue(buffer,bufferlen, &StartBit);
   }

   if((slice_type == 3)){//B
	   /*direct_spatial_mv_pred_flag  B图像在直接预测模式下，1:空间预测，0:时间预测 */
	   bs_read_u(1,buffer, &StartBit);
   }
...
int h264Parser::h264_parser_pps(unsigned char * buffer, unsigned int bufferlen){
   ...
   //========NAL Header=========
    ...
    //========Parser pps=========
    if(nal_unit_type == 8){
    	m_h264_paramter.pps[m_h264_paramter.ppslen].pic_parameter_set_id = bs_read_ue(buffer,bufferlen, &StartBit);// 1 bs_read_ue(v)
    	m_h264_paramter.pps[m_h264_paramter.ppslen].seq_parameter_set_id = bs_read_ue(buffer,bufferlen, &StartBit);// 1 bs_read_ue(v)
    	m_h264_paramter.pps[m_h264_paramter.ppslen].entropy_coding_mode_flag = bs_read_u(1,buffer, &StartBit);// 1 bs_read_u(1)
    	m_h264_paramter.pps[m_h264_paramter.ppslen].pic_order_present_flag = bs_read_u(1,buffer, &StartBit);// 1 bs_read_u(1)
    	m_h264_paramter.pps[m_h264_paramter.ppslen].num_slice_groups_minus = bs_read_ue(buffer,bufferlen, &StartBit) + 1;// 1 bs_read_ue(v)
    	if((m_h264_paramter.pps[m_h264_paramter.ppslen].num_slice_groups_minus-1) > 0){
    		m_h264_paramter.pps[m_h264_paramter.ppslen].slice_group_map_type = bs_read_ue(buffer,bufferlen, &StartBit);
    		if(m_h264_paramter.pps[m_h264_paramter.ppslen].slice_group_map_type == 0){
    			for(int iGroup = 0;iGroup < (m_h264_paramter.pps[m_h264_paramter.ppslen].num_slice_groups_minus-1);iGroup++){
    				m_h264_paramter.pps[m_h264_paramter.ppslen].run_length_minus1 = bs_read_ue(buffer,bufferlen, &StartBit);
    			}
    		}else if(m_h264_paramter.pps[m_h264_paramter.ppslen].slice_group_map_type == 2){
    			for(int iGroup = 0;iGroup < (m_h264_paramter.pps[m_h264_paramter.ppslen].num_slice_groups_minus-1);iGroup++){
    				m_h264_paramter.pps[m_h264_paramter.ppslen].top_left = bs_read_ue(buffer,bufferlen, &StartBit);
    				m_h264_paramter.pps[m_h264_paramter.ppslen].bottom_right = bs_read_ue(buffer,bufferlen, &StartBit);
    			}
    			...
    			m_h264_paramter.pps[m_h264_paramter.ppslen].slice_group_change_direction_flag = bs_read_u(1,buffer, &StartBit);
    			m_h264_paramter.pps[m_h264_paramter.ppslen].slice_group_change_rate_minus1 = bs_read_ue(buffer,bufferlen, &StartBit);
    		}else if(m_h264_paramter.pps[m_h264_paramter.ppslen].slice_group_map_type == 6){
    			m_h264_paramter.pps[m_h264_paramter.ppslen].pic_size_in_map_units_minus1 = bs_read_ue(buffer,bufferlen, &StartBit);
    			for(int i = 0;i < m_h264_paramter.pps[m_h264_paramter.ppslen].pic_size_in_map_units_minus1;i++){
    				m_h264_paramter.pps[m_h264_paramter.ppslen].slice_group_id = bs_read_ue(buffer,bufferlen, &StartBit);
    			}
    		}
    	}

    	m_h264_paramter.pps[m_h264_paramter.ppslen].num_ref_idx_l0_default_active_minus1 = bs_read_ue(buffer,bufferlen, &StartBit);// 1 bs_read_ue(v)
    	m_h264_paramter.pps[m_h264_paramter.ppslen].num_ref_idx_l1_default_active_minus1 = bs_read_ue(buffer,bufferlen, &StartBit);// 1 bs_read_ue(v)
    	m_h264_paramter.pps[m_h264_paramter.ppslen].weighted_pred_flag = bs_read_u(1,buffer, &StartBit);// 1 bs_read_u(1)
    	m_h264_paramter.pps[m_h264_paramter.ppslen].weighted_bipred_idc = bs_read_u(2,buffer, &StartBit);// 1 bs_read_u(2)
    	m_h264_paramter.pps[m_h264_paramter.ppslen].pic_init_qp_minus26 = bs_read_se(buffer,bufferlen, &StartBit);// /* relative to 26 */ 1 bs_read_se(v)
    	m_h264_paramter.pps[m_h264_paramter.ppslen].pic_init_qs_minus26 = bs_read_se(buffer,bufferlen, &StartBit);// /* relative to 26 */ 1 bs_read_se(v)
    	m_h264_paramter.pps[m_h264_paramter.ppslen].chroma_qp_index_offset = bs_read_se(buffer,bufferlen, &StartBit);// 1 bs_read_se(v)
    	m_h264_paramter.pps[m_h264_paramter.ppslen].deblocking_filter_control_present_flag = bs_read_u(1,buffer, &StartBit);// 1 bs_read_u(1)
    	m_h264_paramter.pps[m_h264_paramter.ppslen].constrained_intra_pred_flag = bs_read_u(1,buffer, &StartBit);// 1 bs_read_u(1)
    	m_h264_paramter.pps[m_h264_paramter.ppslen].redundant_pic_cnt_present_flag = bs_read_u(1,buffer, &StartBit);// 1 bs_read_u(1)
...
int h264Parser::h264_parser_sps(unsigned char * buf, unsigned int nLen){
   ...
   //========NAL Header=========
    /*int forbidden_zero_bit=*/bs_read_u(1,buf, &StartBit);
    /*int nal_ref_idc=*/bs_read_u(2,buf, &StartBit);
    int nal_unit_type=bs_read_u(5,buf, &StartBit);

    //========Parser sps=========
    if(nal_unit_type==7)
    {
    		m_h264_paramter.sps[m_h264_paramter.spslen].profile_idc = bs_read_u(8,buf, &StartBit);
    		bs_read_u(8,buf, &StartBit);//固定8字节
    		m_h264_paramter.sps[m_h264_paramter.spslen].level_idc = bs_read_u(8,buf, &StartBit);
    		//表示当前的序列参数集的id。通过该id值，图像参数集pps可以引用其代表的sps中的参数。
    		m_h264_paramter.sps[m_h264_paramter.spslen].seq_parameter_set_id=bs_read_ue(buf,nLen, &StartBit);
        ...
        {
    			m_h264_paramter.sps[m_h264_paramter.spslen].chroma_format_idc = bs_read_ue(buf,nLen, &StartBit);
            if(m_h264_paramter.sps[m_h264_paramter.spslen].chroma_format_idc == 3 ){
            	m_h264_paramter.sps[m_h264_paramter.spslen].separate_colour_plane_flag = bs_read_u(1,buf, &StartBit);
               }
            m_h264_paramter.sps[m_h264_paramter.spslen].bit_depth_luma = bs_read_ue(buf,nLen, &StartBit)+8;
            m_h264_paramter.sps[m_h264_paramter.spslen].bit_depth_chroma = bs_read_ue(buf,nLen, &StartBit)+8;
            m_h264_paramter.sps[m_h264_paramter.spslen].qpprime_y_zero_transform_bypass_flag=bs_read_u(1,buf, &StartBit);
            m_h264_paramter.sps[m_h264_paramter.spslen].seq_scaling_matrix_present_flag = bs_read_u(1,buf, &StartBit);
            ...
            if(m_h264_paramter.sps[m_h264_paramter.spslen].seq_scaling_matrix_present_flag )
            {
                for( int i = 0; i < ( ( m_h264_paramter.sps[m_h264_paramter.spslen].chroma_format_idc  !=  3 ) ? 8 : 12 ); i++ ) {
                    seq_scaling_list_present_flag[i]=bs_read_u(1,buf, &StartBit);
    		...
    		m_h264_paramter.sps[m_h264_paramter.spslen].log2_max_frame_num=bs_read_ue(buf,nLen, &StartBit)+4;
    		m_h264_paramter.sps[m_h264_paramter.spslen].pic_order_cnt_type=bs_read_ue(buf,nLen, &StartBit);

        if(m_h264_paramter.sps[m_h264_paramter.spslen].pic_order_cnt_type == 0){

        	m_h264_paramter.sps[m_h264_paramter.spslen].log2_max_pic_order_cnt_lsb_minus = (bs_read_ue(buf,nLen, &StartBit)+4);
        ...
        {
        	m_h264_paramter.sps[m_h264_paramter.spslen].delta_pic_order_always_zero_flag=bs_read_u(1,buf, &StartBit);
        	m_h264_paramter.sps[m_h264_paramter.spslen].offset_for_non_ref_pic = bs_read_se(buf,nLen, &StartBit);
        	m_h264_paramter.sps[m_h264_paramter.spslen].offset_for_top_to_bottom_field = bs_read_se(buf,nLen, &StartBit);
        	m_h264_paramter.sps[m_h264_paramter.spslen].num_ref_frames_in_pic_order_cnt_cycle = bs_read_ue(buf,nLen, &StartBit);
         for( int i = 0; i < m_h264_paramter.sps[m_h264_paramter.spslen].num_ref_frames_in_pic_order_cnt_cycle; i++ )
        	  m_h264_paramter.sps[m_h264_paramter.spslen].offset_for_ref_frame[i]=bs_read_se(buf,nLen, &StartBit);
        }
        m_h264_paramter.sps[m_h264_paramter.spslen].max_num_ref_frames = bs_read_ue(buf,nLen, &StartBit);
        m_h264_paramter.sps[m_h264_paramter.spslen].gaps_in_frame_num_allowed_flag = bs_read_u(1,buf, &StartBit);
        m_h264_paramter.sps[m_h264_paramter.spslen].pic_width_in_mbs_minus1 = bs_read_ue(buf,nLen, &StartBit);
        m_h264_paramter.sps[m_h264_paramter.spslen].pic_height_in_map_units_minus1 = bs_read_ue(buf,nLen, &StartBit);
        m_h264_paramter.sps[m_h264_paramter.spslen].frame_mbs_only_flag = bs_read_u(1,buf, &StartBit);

        if(!m_h264_paramter.sps[m_h264_paramter.spslen].frame_mbs_only_flag){
        	m_h264_paramter.sps[m_h264_paramter.spslen].mb_adaptive_frame_field_flag = bs_read_u(1,buf, &StartBit);
        }

        m_h264_paramter.sps[m_h264_paramter.spslen].direct_8x8_inference_flag = bs_read_u(1,buf, &StartBit);
        m_h264_paramter.sps[m_h264_paramter.spslen].frame_cropping_flag = bs_read_u(1,buf, &StartBit);
        if(m_h264_paramter.sps[m_h264_paramter.spslen].frame_cropping_flag)
        {
        	m_h264_paramter.sps[m_h264_paramter.spslen].crop_left = bs_read_ue(buf,nLen, &StartBit);
        	m_h264_paramter.sps[m_h264_paramter.spslen].crop_right = bs_read_ue(buf,nLen, &StartBit);
        	m_h264_paramter.sps[m_h264_paramter.spslen].crop_top = bs_read_ue(buf,nLen, &StartBit);
        	m_h264_paramter.sps[m_h264_paramter.spslen].crop_bottom = bs_read_ue(buf,nLen, &StartBit);
        }

        //--------------------------------------vui--------------------------------
        m_h264_paramter.sps[m_h264_paramter.spslen].vui_parameters_present_flag = bs_read_u(1,buf, &StartBit);
        if(m_h264_paramter.sps[m_h264_paramter.spslen].vui_parameters_present_flag)
        {
        	m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.aspect_ratio_info_present_flag = bs_read_u(1,buf, &StartBit);
            if(m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.aspect_ratio_info_present_flag)
            {
            	m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.aspect_ratio_idc=bs_read_u(8,buf, &StartBit);
                if(m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.aspect_ratio_idc==255)
                {
                	m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.sar_width = bs_read_u(16,buf, &StartBit);
                	m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.sar_height = bs_read_u(16,buf, &StartBit);
                }
            }
            m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.overscan_info_present_flag = bs_read_u(1,buf, &StartBit);
            if(m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.overscan_info_present_flag){
            	m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.overscan_appropriate_flag = bs_read_u(1,buf, &StartBit);
            }
            m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.video_signal_type_present_flag = bs_read_u(1,buf, &StartBit);
            if(m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.video_signal_type_present_flag)
            {
            	m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.video_format = bs_read_u(3,buf, &StartBit);
            	m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.video_full_range_flag = bs_read_u(1,buf, &StartBit);
            	m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.colour_description_present_flag = bs_read_u(1,buf, &StartBit);
                if(m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.colour_description_present_flag)
                {
                    m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.colour_primaries = bs_read_u(8,buf, &StartBit);
                    m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.transfer_characteristic = bs_read_u(8,buf, &StartBit);
                    m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.matrix_coeffs = bs_read_u(8,buf, &StartBit);
                }
            }
            m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.chroma_loc_info_present_flag = bs_read_u(1,buf, &StartBit);
            if(m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.chroma_loc_info_present_flag)
            {
            	m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.chroma_sample_loc_type_top_field = bs_read_ue(buf,nLen, &StartBit);
            	m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.chroma_sample_loc_type_bottom_field = bs_read_ue(buf,nLen, &StartBit);
            }

            m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.timing_info_present_flag = bs_read_u(1,buf, &StartBit);
            if(m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.timing_info_present_flag)
            {
            	m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.num_units_in_tick = bs_read_u(32,buf, &StartBit);
            	m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.time_scale = bs_read_u(32,buf, &StartBit);
            	m_h264_paramter.sps[m_h264_paramter.spslen].vui_param.fix_frame_rate_flag = bs_read_u(1,buf, &StartBit);
...
int h264Parser::h264_parser_sei(unsigned char * buffer, unsigned int bufferlen){
	...
	/*int byte = */bs_read_u(8,buffer, &StartBit);//READ_CODE(8,"NO_USE");
...
int h264Parser::h264_scaling_list(int sizeOfScalingList,unsigned char * buf,unsigned int *StartBit,int len){
	...
	for(int j = 0; j < sizeOfScalingList; j++ ) {
		if( nextScale != 0 ) {
			unsigned int delta_scale = bs_read_se(buf,len, StartBit); // 0 | 1  bs_read_se(v)
...
int h264Parser::h264_parse_ref_count(int sliceType,unsigned char * buffer, unsigned int bufferlen,unsigned int *StartBit,int &list_count,int picture_structure){
	...
	if(sliceType != 1){
		...
		num_ref_idx_active_override_flag = bs_read_u(1,buffer, StartBit);
		if(num_ref_idx_active_override_flag){
			ref_count_0 = bs_read_ue(buffer,bufferlen, StartBit) + 1;
			if(sliceType == 3){
				ref_count_1 = bs_read_ue(buffer,bufferlen, StartBit) + 1;
			}else{
				ref_count_1 = 1;
			}
		}
...
int h264Parser::h264_decode_ref_pic_list_reordering(unsigned char * buffer, unsigned int bufferlen,unsigned int *StartBit,int list_count){
    ...
    for (list = 0; list < list_count; list++) {
        if (!bs_read_u(1,buffer, StartBit))    // ref_pic_list_modification_flag_l[01]
            continue;

        for (index = 0; ; index++) {
            unsigned int op = bs_read_ue(buffer,bufferlen, StartBit);
...
int h264Parser::h264_pred_weight_table(unsigned char * buffer, unsigned int bufferlen,unsigned int *StartBit){
    ...
    int luma_log2_weight_denom = bs_read_ue(buffer,bufferlen, StartBit);
...
int opcode;
	if(slice_type == 5){
		bs_read_u(1,buffer, StartBit);
		if(bs_read_u(1,buffer, StartBit)){
			mmco = 1;
		}
		else{
			int i = 0;
			if(bs_read_u(1,buffer, StartBit)){
				for(i = 0;i < 66;i++){
					opcode = bs_read_ue(buffer,bufferlen, StartBit);
